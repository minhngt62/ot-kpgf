...
class LOT(_OT):
    ...
    def fit(self, source: np.ndarray, target: np.ndarray, a=None, b=None, **kwargs) -> np.ndarray:
        ...
        self.Pa_, self.Pb_, self.Pz_, self.P_ = Px, Py, Pz, P
    ...
    def transport(self, source, target, **kwargs) -> np.ndarray:
        n = source.shape[0]
        m = target.shape[0]
        Cx_lot = self.Pa_.T.dot(source) / (self.Pa_.T.dot(np.ones([n, 1])) + 10 ** -20)
        Cy_lot = self.Pb_.dot(target) / (self.Pb_.dot(np.ones([m, 1])) + 10 ** -20)
        transported = source + np.dot(
            np.dot(
                self.Pa_ / np.sum(self.Pa_, axis=1).reshape([n, 1]),
                self.Pz_ / np.sum(self.Pz_, axis=1).reshape([self.n_source_anchors, 1])
            ),
            Cy_lot) - np.dot(self.Pa_ / np.sum(self.Pa_, axis=1).reshape([n, 1]), Cx_lot)
        return transported
